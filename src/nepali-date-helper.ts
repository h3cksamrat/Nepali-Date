import { DateConfig, dateConfigMap } from './date-config'
...
export class NepaliDateHelper {
  public readonly yearMonthDaysMapping: number[][]
  public readonly monthDaysMappings: number[][][]
  private yearDaysMapping: number[][]
  private daysPassed = 0

  constructor(dateConfigMap?: DateConfig)

  constructor() {
    const constructorError = new Error('Invalid constructor arguments')
    let classDateConfigMap: DateConfig = {}
    if (arguments.length === 0) {
      classDateConfigMap = dateConfigMap
    } else if (arguments.length === 1) {
      classDateConfigMap = arguments[0]
    } else {
      throw constructorError
    }

    this.yearMonthDaysMapping = Object.values(classDateConfigMap).map((year) => Object.values(year))
    this.monthDaysMappings = this.yearMonthDaysMapping.map((yearMappings: number[]) => {
      let daySum = 0
    ...
    this.yearDaysMapping = this.yearMonthDaysMapping.map((yearMappings: number[]) => {
      const daysInYear = yearMappings.reduce((acc, x) => acc + x, 0)
      const yearDaysPassed = [daysInYear, this.daysPassed]
      this.daysPassed += daysInYear
   ...
   */
  findPassedDays(year: number, month: number, date: number) {
    try {
      const yearIndex = getYearIndex(year)
      const pastYearDays = this.yearDaysMapping[yearIndex][COMPLETED_DAYS]
      const extraMonth = mod(12, month)
      const extraYear = Math.floor(month / 12)

      const pastMonthDays =
        this.yearDaysMapping[yearIndex + extraYear][COMPLETED_DAYS] -
        pastYearDays +
        this.monthDaysMappings[yearIndex + extraYear][extraMonth][COMPLETED_DAYS]
   ...
   */
  mapDaysToDate(daysPassed: number): IYearMonthDate {
    if (daysPassed < MIN_DAY || daysPassed > MAX_DAY) {
      throw new Error(`The epoch difference is not within the boundaries ${MIN_DAY} - ${MAX_DAY}`)
    }

    const yearIndex = this.yearDaysMapping.findIndex(
      (year) =>
        daysPassed > year[COMPLETED_DAYS] && daysPassed <= year[COMPLETED_DAYS] + year[TOTAL_DAYS]
    )
    const monthRemainder = daysPassed - this.yearDaysMapping[yearIndex][COMPLETED_DAYS]
    const monthIndex = this.monthDaysMappings[yearIndex].findIndex(
      (month) =>
        monthRemainder > month[COMPLETED_DAYS] &&
        monthRemainder <= month[COMPLETED_DAYS] + month[TOTAL_DAYS]
    )
    const date = monthRemainder - this.monthDaysMappings[yearIndex][monthIndex][COMPLETED_DAYS]
  ...
  findPassedDaysAD(year: number, month: number, date: number) {
  ...
  mapDaysToDateAD(daysPassed: number) {
  ...
  convertToAD(bsDateObject: IYearMonthDate): IAdBs {
    try {
      const daysPassed = this.findPassedDays(
        bsDateObject.year,
        bsDateObject.month,
        bsDateObject.date
      )
      const BS = this.mapDaysToDate(daysPassed)
      const AD = this.mapDaysToDateAD(daysPassed)
  ...
  convertToBS(adDateObject: Date): IAdBs {
    try {
      const daysPassed = this.findPassedDaysAD(
  ...
  mapLanguageNumber(dateNumber: string, language: 'en' | 'np'): string {
    return dateNumber
      .split('')
  ...
  format(bsDate: IYearMonthDate, stringFormat: string, language: 'en' | 'np'): string {
    return stringFormat
      .replace(/((\\[MDYd])|D{1,2}|M{1,4}|Y{2,4}|d{1,3})/g, (match, _, matchedString) => {
        switch (match) {
          case 'D':
            return this.mapLanguageNumber(bsDate.date.toString(), language)
          case 'DD':
            return this.mapLanguageNumber(bsDate.date.toString().padStart(2, '0'), language)
          case 'M':
            return this.mapLanguageNumber((bsDate.month + 1).toString(), language)
          case 'MM':
            return this.mapLanguageNumber((bsDate.month + 1).toString().padStart(2, '0'), language)
          case 'MMM':
            return formatObj[language].month.short[bsDate.month]
          case 'MMMM':
            return formatObj[language].month.long[bsDate.month]
          case 'YY':
            return this.mapLanguageNumber(bsDate.year.toString().slice(-2), language)
          case 'YYY':
            return this.mapLanguageNumber(bsDate.year.toString().slice(-3), language)
          case 'YYYY':
            return this.mapLanguageNumber(bsDate.year.toString(), language)
          case 'd':
            return this.mapLanguageNumber(bsDate.day?.toString() || '0', language)
          case 'dd':
  ...
  parse(dateString: string): IYearMonthDate {
